...
function useNearestAddress(value: PlacePickerValue | null, permission: PickerPermission, paused: boolean): string {
  ...
  return address.key === key && allowed ? address.text : '';
}
...
function MapPicker({ value, radiusM, placeName, onChange, limitCenter, limitRadiusM }: PlacePickerProps) {
  ...
  // 검색·프리셋으로 고른 장소(그 근처에서 미세 조정하면 이름을 유지한다)
  const picked = useRef<{ name: string; lat: number; lng: number } | null>(
    value?.name ? { name: value.name, lat: value.lat, lng: value.lng } : null,
  );

  const moveTo = useCallback(
    (p: GeoPoint) => {
      setCenter(p);
      mapRef.current?.animateToRegion(regionAround(p, circleRadius), ANIMATE_MS);
    },
    [circleRadius],
  );
  ...
  const onRegionChangeComplete = useCallback((region: Region, details: Details) => {
    ...
    setNotice(null);
    onChangeRef.current(name ? { lat: c.lat, lng: c.lng, name } : { lat: c.lat, lng: c.lng });
  }, []);
  ...
  const onPick = useCallback(
    (r: PlaceSearchResult) => {
      ...
      setNotice(null);
      onChangeRef.current({ lat: p.lat, lng: p.lng, name: r.name });
      moveTo(p);
    },
    [moveTo],
  );
  ...
  const onFix = useCallback(
    (fix: Fix) => {
      interacted.current = true;
      picked.current = null;
      onChangeRef.current({ lat: fix.point.lat, lng: fix.point.lng });
      moveTo(fix.point);
...
function FormPicker(props: PlacePickerProps) {
  ...
  const mode = placeSearchMode(Platform.OS, HAS_KAKAO_KEY, permission);
  const address = useNearestAddress(value, permission, false);
  ...
  return <PlacePickerFallback {...props} header={header} />;
}
