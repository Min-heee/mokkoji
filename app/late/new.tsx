 ...
 *               시간 뒤로 미루기(최대 3시간)·장소만 — 동결된 항목은 비활성 + 이유(LB_EDIT_FROZEN 문구). 제목·메모는 updateMemo.
 *
 * 규칙
...
/** 시작 후 미루기 상한(fakeApi FAKE_MAX_POSTPONE_MS 와 같다) */
const MAX_POSTPONE_MS = 180 * MIN;
const TITLE_MAX = 40;
...
function Form({ mode, prefill, initialLive }: FormProps) {
  ...
  if (parsed.status === 'incomplete' || parsed.status === 'invalid') timeIssue = appointmentInputHint(parsed.status);
    ...
    if (started && appt && meetAtMs < appt.meetAtMs) timeIssue = errorMessage('LB_POSTPONE_ONLY');
    else if (started && appt && meetAtMs > appt.meetAtMs + MAX_POSTPONE_MS) timeIssue = errorMessage('LB_POSTPONE_TOO_FAR');
  ...
  const goPlace = () => {
    openPlaceDraft({ value: pin, radiusM: radiusValid ? radiusM : 100, placeName: placeName.trim() });
  ...
  const onSubmitPress = () => {
    if (!canSubmit) return;
    // 시작 전 걸 포인트 인상: 들어온 친구 전원에게 차액이 자동으로 더 걸린다 — 한 번 묻는다
    if (isEdit && appt && !started && policy.stake > appt.policy.stake && memberCount > 1) {
      ...
      >
        {isEdit && started ? (
          <Card>
            ...
            <Text style={styles.help}>시간은 뒤로 미루기만(최대 3시간), 장소는 바꿀 수 있어요. 걸 포인트·지각 규칙·명단은 그대로예요.</Text>
          </Card>
        ) : isEdit ? (
        ...
        {isEdit && started && appt ? (
          <Text style={styles.help}>
            {formatKoreanDateTime(appt.meetAtMs, appt.tz)}에서 뒤로 미루기만 할 수 있어요(최대 3시간).
          </Text>
        ) : null}
